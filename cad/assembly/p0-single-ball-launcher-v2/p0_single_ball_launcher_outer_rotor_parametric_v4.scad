// P0 single-ball football launcher outer-rotor assembly placeholder v3 (parametric)
// Units: mm
...
motor_body_len = 53.5;        // 6354 placeholder length
motor_shaft_d = 10;
motor_shaft_len = 14;
...
module bottom_face_mount(){
  // intentionally only under the motor; no top-face hanging plate
  color("#9ca3af") {
    ...
    for(a=[90,210,330]) {
      rotate([0,0,a])
        translate([ring_od/2 - cutout_w/2 + 8, 0, 0])
          cube([cutout_w, motor_body_d + cutout_extra, ring_h + 6], center=true);
    }
  }
