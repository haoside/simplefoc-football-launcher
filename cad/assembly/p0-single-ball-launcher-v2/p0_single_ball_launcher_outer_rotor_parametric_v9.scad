// P0 single-ball football launcher outer-rotor assembly placeholder v9 (parametric)
// Units: mm
...
// - explicit side support plates integrated into the mainline head-to-frame spacing
...
use_side_support_plates = true;
head_to_frame_gap = 26;      // nominal spacing from outer mount plate to side support plate
side_plate_w = 120;
side_plate_h = 180;
side_plate_t = 8;
...
module outer_side_support_plates(){
  if (use_side_support_plates) {
    color([0.58,0.62,0.70,0.90])
      translate([0,0,head_z])
        for(a=[30,150,270])
          rotate([0,0,a])
