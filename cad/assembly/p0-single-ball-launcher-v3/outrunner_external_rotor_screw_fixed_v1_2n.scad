// SimpleFOC football launcher - external-rotor screw-fixed embedded mount layout v1.2n
...
// - external rotor can is the spinning contact surface; black screws fix the non-rotating stator/blue endbell side only
...
show_black_fixing_screws  = true;
show_stator_mount_bosses  = true;
show_rotating_can_band    = true;
...
black_screw_head_d       = 8.0;
black_screw_head_h       = 3.2;
stator_boss_d            = 12;
stator_boss_h            = 8;
rotating_can_band_w      = 18;
rotating_can_gap         = 2.5;
...
module four_screw_side_tab_heads_photo2() {
  if (show_black_fixing_screws)
    translate([0,0,motor_station_z])
      for (a=[30,150]) rotate([0,0,a])
        for (x=[-four_screw_tab_pitch_x/2, four_screw_tab_pitch_x/2])
          for (z=[-four_screw_tab_pitch_z/2, four_screw_tab_pitch_z/2])
            translate([x, motor_center_r + motor_d/2 + four_screw_tab_t + 29.8, z])
              rotate([90,0,0]) black_screw_head();
}

module bridge_strap_screw_heads_photo2() {
  if (show_black_fixing_screws)
    translate([0,0,motor_station_z]) {
      rotate([0,0,-90])
        for (x=[-top_bridge_w/3, top_bridge_w/3])
          translate([x, motor_center_r + motor_d/2 + 14 + top_bridge_d/2 + 1.8, 0])
            rotate([90,0,0]) black_screw_head();
      for (a=[30,150]) rotate([0,0,a])
        for (x=[-lower_cradle_w/3, lower_cradle_w/3])
          translate([x, motor_center_r + motor_d/2 + 8 + lower_cradle_d/2 + 1.8, -motor_d/2 - 14])
            rotate([90,0,0]) black_screw_head();
    }
}
...
module black_screw_head() {
  color([0.01,0.01,0.012,1]) cylinder(d=black_screw_head_d, h=black_screw_head_h, center=true);
}

module stator_45_fixing_bosses() {
  if (show_stator_mount_bosses)
    for (x=[-mount_45_pitch/2, mount_45_pitch/2])
      for (z=[-mount_45_pitch/2, mount_45_pitch/2])
        translate([x, motor_center_r + motor_d/2 + stator_boss_h/2 + 6, z])
          rotate([90,0,0]) cylinder(d=stator_boss_d, h=stator_boss_h, center=true);
}

module black_45_screw_heads() {
  if (show_black_fixing_screws)
    for (x=[-mount_45_pitch/2, mount_45_pitch/2])
      for (z=[-mount_45_pitch/2, mount_45_pitch/2])
        translate([x, motor_center_r + motor_d/2 + stator_boss_h + black_screw_head_h/2 + 8, z])
          rotate([90,0,0]) black_screw_head();
}

module external_rotor_visual() {
  // 6354 outrunner: silver/gold can is the rotating outer shell; blue/end side is fixed by black screws.
  rotate([0,90,0]) {
    color([0.90,0.86,0.72,1]) cylinder(d=motor_d, h=motor_len, center=true);
    if (show_rotating_can_band) {
      color([0.95,0.95,0.92,1]) translate([0,0,-motor_len*0.18]) cylinder(d=motor_d+0.6, h=rotating_can_band_w, center=true);
      color([0.10,0.34,0.92,1]) translate([0,0,motor_len/2-5]) cylinder(d=motor_d+1.2, h=10, center=true);
      color([0.10,0.34,0.92,1]) translate([0,0,-motor_len/2+5]) cylinder(d=motor_d+1.2, h=10, center=true);
    }
  }
}
...
module one_motor_station() {
  ...
  stator_45_fixing_bosses();
  black_45_screw_heads();
...
module tilted_launch_cluster() {
  ...
  four_screw_side_tab_heads_photo2();
  bridge_strap_screw_heads_photo2();
  translate([0,0,motor_station_z]) {
