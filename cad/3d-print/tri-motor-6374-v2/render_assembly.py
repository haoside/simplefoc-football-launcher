...
def build_assembly():
    ...
    for i in range(N_MOTORS):
        ...
        boss = Manifold.cylinder(FRAME_LEN, BOSS_EXTRA / 2, BOSS_EXTRA / 2, SEGMENTS)
        boss = boss.translate([cx, cy, 0])
        tube = tube + boss

        boss_hole = Manifold.cylinder(FRAME_LEN + 2, MOTOR_D / 2 + 0.5, MOTOR_D / 2 + 0.5, SEGMENTS)
    ...
    for i in range(N_MOTORS):
        ...
        rib = Manifold.cylinder(FRAME_LEN, 4, 4, 16)
        rib = rib.translate([(TUBE_OR + BOSS_OR) / 2, 0, 0])
        rib = rib.rotate([0, 0, a_mid])
        tube = tube + rib
