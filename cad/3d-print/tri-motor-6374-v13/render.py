...
def render(parts, view, filename, title):
    ...
    legend_items = [
        plt.Rectangle((0, 0), 1, 1, fc=COLORS['spider_body'], label='Spider Body (mimics reference)'),
...
def build():
    ...
    for i in range(N_MOTORS):
        angle_deg = i * 120
        angle_rad = math.radians(angle_deg)
        cx = (CRADLE_R - 15) * math.cos(angle_rad)
        cy = (CRADLE_R - 15) * math.sin(angle_rad)
        can_r = MOTOR_D / 2 + 1
        ...
        stator_x = (CRADLE_R + 25) * math.cos(angle_rad)
        stator_y = (CRADLE_R + 25) * math.sin(angle_rad)
...
def main():
    ...
    parts = build()
    render(parts, 'iso', 'assembly_v13_iso.png',
           f'Football Launcher v13 — Visual mimicry of reference\n(ignoring axis angle, focus on shape)')
    render(parts, 'top', 'assembly_v13_top.png',
           f'Football Launcher v13 — Top View (3 cradles @ 120°)')
