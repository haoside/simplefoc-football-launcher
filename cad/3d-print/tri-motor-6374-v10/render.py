...
TUBE_LEN = 120
N_MOTORS = 3
MOTOR_CENTER_R = TUBE_OR + 5
CRADLE_HUB_OD = MOTOR_D + 12
WHEEL_RIM_OR = MOTOR_CENTER_R + 45
SEGMENTS = 64
...
def render(parts, view, filename, title):
    ...
    legend_items = [
        ...
        plt.Rectangle((0, 0), 1, 1, fc=COLORS['motor_can'], label='Motor Can (180° reversed)'),
...
def build():
    ...
    for i in range(N_MOTORS):
        ...
        # hub
        hub_outer = C(MOTOR_L + 12, CRADLE_HUB_OD / 2).rotate([0, math.pi / 2, 0])
        hub_inner = C(MOTOR_L + 12.4, MOTOR_D / 2 + 0.5).rotate([0, math.pi / 2, 0])
        ...
        for j in range(3):
            ...
            spoke = Manifold.cube((10, spoke_len, 16), center=True)
            spoke = spoke.rotate([sa, 0, 0])
            spoke = spoke.rotate([0, 0, angle_deg])
            spoke = spoke.translate([cx + spoke_offset * math.cos(sa + angle_rad),
                                     cy + spoke_offset * math.sin(sa + angle_rad), 0])
            spokes = spokes + spoke

        # 外圈
        rim = C(10, WHEEL_RIM_OR / 2).rotate([0, math.pi / 2, 0]) - \
              C(11, (WHEEL_RIM_OR - 24) / 2).rotate([0, math.pi / 2, 0])
    ...
    for i in range(N_MOTORS):
        ...
        cx = MOTOR_CENTER_R * math.cos(angle_rad)
        cy = MOTOR_CENTER_R * math.sin(angle_rad)
        # can 朝管内（180° 反向 = -X 方向相对电机中心）
        # can 长 84mm，半数在管内
        can_r = MOTOR_D / 2 + 1
        can = C(MOTOR_L + 10, can_r) - C(MOTOR_L + 10.4, MOTOR_D / 2 + 0.3)
...
def main():
    ...
    parts = build()
    render(parts, 'iso', 'assembly_v10_iso.png',
           f'Football Launcher v10 — INTEGRATED 180° spider\n3× 6374 radial | Ball 220mm')
    render(parts, 'top', 'assembly_v10_top.png',
           f'Football Launcher v10 — Top View (3 cradle wheels @ 120°)')
