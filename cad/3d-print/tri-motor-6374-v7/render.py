...
CRADLE_RIM_OR = MOTOR_CENTER_R + 50
CRADLE_HUB_OD = MOTOR_D + 8
CRADLE_HUB_LEN = MOTOR_L + 8
CRADLE_THICK = 12
CRADLE_N_SPOKES = 3
N_MOTORS = 3
SEGMENTS = 64
...
def render(parts, view, filename, title):
    ...
    legend_items = [
        ...
        plt.Rectangle((0, 0), 1, 1, fc=COLORS['motor_cradle'], label='Motor Cradle (radial disc)'),
...
def build():
    ...
    for i in range(N_MOTORS):
        ...
        for j in range(CRADLE_N_SPOKES):
            ...
            spoke = Manifold.cube((CRADLE_THICK, spoke_len, 12), center=True)
    ...
    for i in range(N_MOTORS):
        ...
        angle_rad = math.radians(angle_deg)
        # can 伸出 cradle 朝球方向
        cx = (MOTOR_CENTER_R - 20) * math.cos(angle_rad)
        cy = (MOTOR_CENTER_R - 20) * math.sin(angle_rad)
        can_r = MOTOR_D / 2 + 1
...
def main():
    ...
    parts = build()
    render(parts, 'iso', 'assembly_v7_iso.png',
           f'Football Launcher v7 — RADIAL disc cradle (refined)\n3× 6374 perpendicular to tube | Ball 220mm')
    render(parts, 'top', 'assembly_v7_top.png',
           f'Football Launcher v7 — Top View')
